...
class PageRankNibbleApp(QMainWindow):
    ...
    def run_page_rank_nibble(self):
        ...
        c = 0.09
        try:
            seed, cluster = page_rank_nibble(self.graph, beta, "unweighted", product_index)
        except Exception as e:
...
if __name__ == "__main__":
    ...
    graph_path = os.path.join(DATA_DIR, "graph_40-510.pkl")
    dict_path = os.path.join(DATA_DIR, "products_dict_40-510.pkl")
    app = QApplication(sys.argv)
    window = PageRankNibbleApp(graph_path=graph_path, dict_path=dict_path, **client_params)
    window.show()
    sys.exit(app.exec_())
